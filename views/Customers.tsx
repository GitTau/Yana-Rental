...
import { UserPlus, UserCheck, Shield, CheckCircle, XCircle, AlertTriangle, Mail, Phone, Calendar, MapPin, CalendarDays, Smartphone, FileText } from 'lucide-react';
      ...
      <Modal isOpen={!!selectedCustomer} onClose={() => setSelectedCustomer(null)} title="Rider Profile">
        {selectedCustomer && (
          <div className="space-y-6">
            ...
            <div className="space-y-3">
               <h4 className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Documentation</h4>
               <div className="bg-gray-50 p-4 rounded-2xl border border-gray-100 grid grid-cols-2 gap-4">
                  <div>
                    <p className="text-[8px] font-black text-gray-400 uppercase mb-1">Aadhar</p>
                    <p className="text-xs font-bold text-gray-800">{selectedCustomer.aadharNo || 'Missing'}</p>
                  </div>
                  <div>
                    <p className="text-[8px] font-black text-gray-400 uppercase mb-1">PAN</p>
            ...
            <div className="pt-4 border-t border-gray-100 flex gap-3">
               <button className="flex-1 py-3 bg-slate-900 text-white rounded-xl text-[10px] font-black uppercase tracking-widest active:scale-95 transition-all">History</button>
               {!selectedCustomer.kycStatus && (
                 <button className="flex-1 py-3 bg-emerald-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest active:scale-95 transition-all">Verify KYC</button>
               )}
            </div>
          </div>
        )}
